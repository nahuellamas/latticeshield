...
| Month | Milestone |
|---|---|
| 23 | **Browser SDK Hardening** — SRI hash auto-generated by `npm run build` (`latticeshield_bg.wasm.sha384`); TypeScript coverage in CI skill; cloud-side heartbeat signature verification; `Zeroizing<Vec<u8>>` for `ikm` in `derive_session_key` |
